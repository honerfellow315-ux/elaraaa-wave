      ...
      {selected && (
        ...
        >
          <div className="glass-card max-w-lg w-full p-6 relative" onClick={(e) => e.stopPropagation()}>
            ...
            <h3 className="text-lg font-bold text-navy pr-8">{selected.brand}</h3>
            <p className="mt-1 text-xs text-text-muted">
              {selected.name} · {selected.email} · Size: {selected.size} · Qty: {selected.quantity}
            </p>
            <p className="mt-4 text-sm text-navy whitespace-pre-wrap leading-relaxed">{selected.brief}</p>
          </div>
        </div>
      )}
    </>

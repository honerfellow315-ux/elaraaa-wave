import { createFileRoute } from "@tanstack/react-router";
import { Panel, Btn } from "@/components/PanelUI";

export const Route = createFileRoute("/account/settings")({ component: Settings });

function Settings() {
  return (
    <div className="space-y-6">
      <Panel title="Preferences">
        <div className="space-y-3">
          {[
            "Email order confirmations",
            "SMS delivery updates",
            "Marketing emails",
            "Hydration reminders",
          ].map((t, i) => (
            <label key={t} className="flex items-center justify-between px-4 py-3 rounded-xl bg-white/70">
              <span className="text-sm text-navy font-medium">{t}</span>
              <input type="checkbox" defaultChecked={i !== 2} className="h-5 w-9 appearance-none rounded-full bg-navy/20 checked:bg-brand relative transition-all cursor-pointer before:absolute before:top-0.5 before:left-0.5 before:h-4 before:w-4 before:rounded-full before:bg-white before:transition-all checked:before:translate-x-4" />
            </label>
          ))}
        </div>
      </Panel>
      <Panel title="Change password">
        <div className="grid md:grid-cols-3 gap-4">
          <F l="Current password" />
          <F l="New password" />
          <F l="Confirm password" />
        </div>
        <div className="mt-6"><Btn>Update password</Btn></div>
      </Panel>
    </div>
  );
}

function F({ l }: { l: string }) {
  return (
    <div>
      <label className="text-xs font-bold tracking-widest text-text-muted uppercase">{l}</label>
      <input type="password" className="mt-2 w-full h-11 px-4 rounded-xl bg-white/80 border border-white/80 text-navy focus:outline-none focus:border-blue" />
    </div>
  );
}
